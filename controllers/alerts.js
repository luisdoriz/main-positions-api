...
exports.generateAlerts = async ({
  ...
  if (timeLimit) {
    ...
    if (excededTimeLimit) {
      ...
      const person = await Persons.readPerson(idPerson);
      const rArea = await Areas.readArea(area);
      let payload = `${person.name} ${person.firstLastName} ${person.secondLastName} ha excedido el tiempo permitido en el área "${rArea.name}" con un total de ${timeInArea} minutos`;
      Alerts.createAlert({
        payload,
        idArea: area,
        idPerson,
  ...
  if (maxCapacity) {
    ...
    if (excededOcuppancy) {
      ...
      const rArea = await Areas.readArea(area);
      let payload = `Se ha excedido el limite de personas permitidas en el área "${rArea.name}" con un total de ${numPeople} personas`;
      Alerts.createAlert({
        payload,
        idArea: area,
        idPerson,
